...
def home():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return render_template(
                "error.html",
                msg=f"Model detection error: {e}"
            )

        gps_data = extract_gps_metadata(filepath)

        if gps_data['Latitude'] is None and gps_data["Longitude"] is None:
